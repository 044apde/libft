...
/*   Updated: 2023/07/13 20:07:57 by shikim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *ft_strtrim(char const *s1, char const *set) {
  ...
  if (*s1 == '\0') {
    ...
    if (trimmed_str == 0) return (0);
    trimmed_str[0] = '\0';
    return (trimmed_str);
  }
  start = 0;
  end = ft_strlen(s1) - 1;
  while (s1[start] != '\0' && ft_strchr(set, s1[start])) ++start;
  while (end > start && ft_strchr(set, s1[end])) --end;
